...
const getQueries = (originalSchema: Schema, newSchema: Schema, force: boolean = false) => {
	...
	if (!force) {
		...
		if (problemActions.length > 0) {
			throw new Error(
				`There are problematic actions, please fix them first: ${JSON.stringify(problemActions, null, 2)}`
			)
		}
	}
